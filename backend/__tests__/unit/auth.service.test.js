...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('throws 409 if email already exists', async () => {
      ...
      await expect(
        authService.register({ name: 'X', email: 'exists@example.com', password: 'Test@1234' })
      ).rejects.toThrow(new ApiError(409, 'Email already registered'));
    });
  });
  ...
  describe('login', () => {
    ...
    it('throws 429 when the account is locked', async () => {
      ...
      await expect(
        authService.login({ email: 'locked@example.com', password: 'Test@1234' })
    ...
    it('throws 403 when the account is deactivated', async () => {
      ...
      await expect(
        authService.login({ email: 'inactive@example.com', password: 'Test@1234' })
