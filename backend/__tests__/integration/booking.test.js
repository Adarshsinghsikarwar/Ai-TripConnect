...
beforeAll(async () => {
  ...
  // Register traveler
  await request(app).post('/api/v1/auth/register').send({ name: 'Traveler', email: 'traveler@test.com', password: 'Test@1234' });
  const tLogin = await request(app).post('/api/v1/auth/login').send({ email: 'traveler@test.com', password: 'Test@1234' });
  travelerToken = tLogin.body.data.accessToken;

  // Register provider user
  await request(app).post('/api/v1/auth/register').send({ name: 'Provider', email: 'provider@test.com', password: 'Test@1234' });
  const pLogin = await request(app).post('/api/v1/auth/login').send({ email: 'provider@test.com', password: 'Test@1234' });
  ...
  await User.findByIdAndUpdate(providerUserId, { $addToSet: { roles: 'provider' } });
});
...
describe('POST /api/v1/bookings — create booking request', () => {
  ...
  it('400 — rejects startDate after endDate', async () => {
    const res = await request(app)
      .post('/api/v1/bookings')
      .set('Authorization', `Bearer ${travelerToken}`)
      .send({ provider: providerId, startDate: '2026-03-15', endDate: '2026-03-10' });
    expect(res.status).toBe(400);
  });
});
...
describe('POST /api/v1/bookings/:id/verify-payment — signature check', () => {
  it('400 — rejects a fake payment signature', async () => {
    ...
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${travelerToken}`)
      .send({ orderId: 'order_fake', paymentId: 'pay_fake', signature: 'wrong_sig' });
    expect(res.status).toBe(400);
...
describe('PATCH /api/v1/bookings/:id/cancel', () => {
  ...
  it('403 — a random user cannot cancel someone else\'s booking', async () => {
    ...
    await request(app).post('/api/v1/auth/register').send({ name: 'Stranger', email: 'stranger@test.com', password: 'Test@1234' });
    const stranger = await request(app).post('/api/v1/auth/login').send({ email: 'stranger@test.com', password: 'Test@1234' });
