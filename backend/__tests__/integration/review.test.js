...
beforeAll(async () => {
  ...
  // Setup traveler
  await request(app).post('/api/v1/auth/register').send({ name: 'Traveler', email: 'traveler@rev.com', password: 'Test@1234' });
  const tLogin = await request(app).post('/api/v1/auth/login').send({ email: 'traveler@rev.com', password: 'Test@1234' });
  travelerToken = tLogin.body.data.accessToken;
  travelerId = tLogin.body.data.userId;

  // Setup provider
  await request(app).post('/api/v1/auth/register').send({ name: 'Guide', email: 'guide@rev.com', password: 'Test@1234' });
  const pLogin = await request(app).post('/api/v1/auth/login').send({ email: 'guide@rev.com', password: 'Test@1234' });
...
describe('POST /api/v1/reviews', () => {
  it('201 — traveler can review a completed booking', async () => {
    const res = await request(app)
      ...
      .send({ booking: completedBookingId, rating: 5, comment: 'Excellent guide!' });
...
describe('GET /api/v1/reviews/provider/:providerId', () => {
  it('200 — returns all reviews for a provider (public, no auth)', async () => {
    ...
    const res = await request(app).get(`/api/v1/reviews/provider/${providerId}`);
