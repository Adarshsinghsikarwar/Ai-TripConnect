...
beforeAll(async () => {
  ...
  await request(app).post('/api/v1/auth/register').send({ name: 'User1', email: 'u1@test.com', password: 'Test@1234' });
  const login1 = await request(app).post('/api/v1/auth/login').send({ email: 'u1@test.com', password: 'Test@1234' });
  accessToken = login1.body.data.accessToken;

  await request(app).post('/api/v1/auth/register').send({ name: 'User2', email: 'u2@test.com', password: 'Test@1234' });
  const login2 = await request(app).post('/api/v1/auth/login').send({ email: 'u2@test.com', password: 'Test@1234' });
...
describe('GET /api/v1/trips', () => {
  ...
  it('supports ?destination filter', async () => {
    ...
    await request(app)
      .post('/api/v1/trips')
      .set('Authorization', `Bearer ${accessToken}`)
      .send({ ...validTrip, title: 'Manali Winter Trip', destination: 'Manali' });
