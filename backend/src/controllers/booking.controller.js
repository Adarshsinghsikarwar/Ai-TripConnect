...
const createBooking = asyncHandler(async (req, res) => {
  const booking = await bookingService.createBookingRequest(req.userId, req.body);
...
const verifyPayment = asyncHandler(async (req, res) => {
  const booking = await bookingService.verifyAndRecordPayment(req.params.id, req.body);
  res.status(200).json(new ApiResponse(200, booking, 'Payment verified'));
});

const cancelBooking = asyncHandler(async (req, res) => {
  const booking = await bookingService.cancelBooking(req.params.id, req.userId, req.body.reason);
...
const markCompleted = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, booking, 'Booking marked completed'));
});
