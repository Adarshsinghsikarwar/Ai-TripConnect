...
const registerProvider = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(new ApiResponse(201, provider, 'Provider profile created, pending verification'));
});
...
const uploadPhotos = asyncHandler(async (req, res) => {
  ...
  const provider = await providerService.addPhotos(req.params.id, req.userId, urls);
  res.status(200).json(new ApiResponse(200, provider, 'Photos uploaded'));
});

const setVerification = asyncHandler(async (req, res) => {
  const provider = await providerService.setVerification(req.params.id, req.body.status);
  res.status(200).json(new ApiResponse(200, provider, 'Verification status updated'));
});

export { registerProvider, getProvider, searchProviders, smartSearch, updateProvider, uploadPhotos, setVerification };
export default { registerProvider, getProvider, searchProviders, smartSearch, updateProvider, uploadPhotos, setVerification };