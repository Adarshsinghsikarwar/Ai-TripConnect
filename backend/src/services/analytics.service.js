...
class AnalyticsService {
  getAdminDashboard() {
    return bookingRepo.aggregate([
      {
        $facet: {
          bookingsByStatus: [{ $group: { _id: '$status', count: { $sum: 1 } } }],
          revenueByMonth: [
  ...
  getTopRatedProviders() {
    return reviewRepo.aggregate([
      { $group: { _id: '$provider', avgRating: { $avg: '$rating' }, reviewCount: { $sum: 1 } } },
      { $sort: { avgRating: -1 } },
      { $limit: 10 },
      {
      ...
      { $unwind: '$providerInfo' },
      { $project: { avgRating: 1, reviewCount: 1, 'providerInfo.title': 1, 'providerInfo.serviceType': 1 } },
    ]);
  }
