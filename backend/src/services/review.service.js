...
class ReviewService {
  async createReview(travelerId, { booking: bookingId, rating, comment }) {
    ...
    if (String(booking.traveler._id || booking.traveler) !== String(travelerId))
      throw new ApiError(
        403,
  ...
  async summarizeForProvider(providerId) {
    ...
    if (withComments.length < 3) {
      return {
        ...
        reason: "Not enough written reviews to summarize yet",
      };
    }
    ...
    try {
      ...
      if (err instanceof AINotConfiguredError) {
        throw new ApiError(
          503,
          "Review summarization is not configured on this server"
        );
      }
