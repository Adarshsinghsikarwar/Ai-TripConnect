...
class BookingService {
  async createBookingRequest(
    travelerId,
    { provider: providerId, startDate, endDate, trip }
  ) {
  ...
  async respondToRequest(bookingId, providerUserId, decision) {
    ...
    const isProvider =
      String(
        booking.provider.user?._id || booking.provider.user || booking.provider
    ...
    if (booking.status !== "requested")
      throw new ApiError(409, "This booking is no longer pending");
    ...
    const updated = await bookingRepo.updateStatusIfCurrent(
      bookingId,
      "requested",
      newStatus
    );
    ...
    await notificationService.notify({
      ...
      type:
        newStatus === "confirmed" ? "booking_confirmed" : "booking_cancelled",
      title:
        newStatus === "confirmed" ? "Booking confirmed!" : "Booking rejected",
      body:
        newStatus === "confirmed"
  ...
  async markCompleted(bookingId, providerUserId) {
    ...
    if (!updated)
      throw new ApiError(
        409,
        "Booking must be ongoing (paid) before it can be completed"
      );
    return updated;
  }
